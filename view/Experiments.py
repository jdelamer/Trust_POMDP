...
class Experiments(Toplevel):
    ...
    def __init__(self, gui, new=True, **kw):
        ...
        """ Define goal """
        Label(self, text="Select the positions of the goals").grid(row=22, column=0, columnspan=4)
        self.goal_x_spinbox = Spinbox(self, from_=0, to_=self.x_spinbox.get(), width=3)
        self.goal_x_spinbox.grid(row=23, column=1)
        self.goal_y_spinbox = Spinbox(self, from_=0, to_=self.y_spinbox.get(), width=3)
        self.goal_y_spinbox.grid(row=23, column=3)

        """ Define number of runs """
        Label(self, text="Select the number of runs").grid(row=26, column=0, columnspan=4)
        Label(self, text="Nb").grid(row=27, column=0, columnspan=2)
        self.run_spinbox = Spinbox(self, from_=0, to_=2, width=3)
        self.run_spinbox.grid(row=27, column=2, columnspan=2)
        self.run_spinbox_pos = []

        """ Coordination or not """
        Label(self, text="Coordination").grid(row=28, column=0, columnspan=2)
        self.coord = IntVar()
        self.coord_button = Checkbutton(self,  text='Coordination', variable=self.coord, onvalue=1, offvalue=0).grid(row=28, column=2, columnspan=2)


        """ Define validation button """
        Button(self, text="Validate", command=self.validate).grid(row=30, column=0, columnspan=4)
    ...
    def validate(self):
        ...
        if self.coord.get() == 0:
            coordination = False
        self.gui.new_experiments(int(self.agent_spinbox.get()), positions_agents, position_goal,
                                  int(self.adv_spinbox.get()), int(self.run_spinbox.get()), coordination)
        self.gui.maze = True
        self.destroy()
